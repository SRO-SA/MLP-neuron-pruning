...
#
# Selector baseline comparison: 4 selectors × 4 targets × 2 datasets = 32 runs.
# All configs use pure_delete pruning — only the channel-selection strategy varies.
#
# Selectors:
...
#   activation_score – activation × down-column-norm             (needs calib data)
...
#   SMOKE=1   bash scripts/run_moe_selector_baseline_comparison.sh # 4 configs (one per selector, target2×wikitext2)
#   DRY_RUN=1 bash scripts/run_moe_selector_baseline_comparison.sh # list 32, do not run
#
# Env overrides:
#   SMOKE=1             Run 4 representative configs only (one per selector, target2×wikitext2)
...
EXPECTED_TOTAL=32   # 4 selectors × 4 targets × 2 datasets
...
# ── Collect all configs ───────────────────────────────────────────────────────
# Order: selector outer, then target, then dataset — natural alphabetical works
# since selector names sort: activation_score, down_norm, random, rmsnorm_bound.
# We want a specific selector order, so enumerate explicitly.
...
echo "[sel] All configs valid."

# ── SMOKE mode: 4 configs — one per selector, target2 × wikitext2 ────────────
if [ "${SMOKE}" = "1" ]; then
    echo "[sel] SMOKE=1: selecting 4 configs (one per selector, target2×wikitext2)."
...
for cfg_path in "${RUN_CONFIGS[@]}"; do
    ...
    if [ ${exit_code} -ne 0 ]; then
        echo "[sel] ✗ exit code ${exit_code}"
        run_ok=0
    fi
    if log_has_error "${run_log}"; then
        echo "[sel] ✗ error indicators found in log"
        run_ok=0
    fi
    if [ ${#NEW_MAIN_CSVS[@]} -eq 0 ]; then
        echo "[sel] ✗ no main CSV created (per-layer CSVs alone do not count)"
        run_ok=0
    fi

    if [ ${run_ok} -eq 1 ]; then
        echo "[sel] ✓ ${cfg_name} done in ${elapsed}s  (main CSVs: ${#NEW_MAIN_CSVS[@]})"
        ...
        echo "[sel] ✗ ${cfg_name} FAILED after ${elapsed}s"
        FAILED=$(( FAILED + 1 ))
